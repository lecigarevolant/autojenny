        ...
        .. code-block:: python
            ...
            async def main() -> None:
                ...
                agent_team = RoundRobinGroupChat(
                    [web_surfer_agent], max_turns=3)

                # Run the team and stream messages to the console
                stream = agent_team.run_stream(
                    task="Navigate to the AutoGen readme on GitHub.")
    ...
    async def _generate_reply(self, cancellation_token: CancellationToken) -> UserContent:
        ...
        if self._model_client.model_info["vision"]:
            ...
            history.append(UserMessage(
                content=[text_prompt, AGImage.from_pil(scaled_screenshot)], source=self.name))
        else:
...
# Describe the viewport of the new page in words
    ...
    async def _get_ocr_text(
        ...
        if isinstance(image, PIL.Image.Image):
            ...
            scaled_screenshot = pil_image.resize(
                (self.MLM_WIDTH, self.MLM_HEIGHT))
            pil_image.close()
