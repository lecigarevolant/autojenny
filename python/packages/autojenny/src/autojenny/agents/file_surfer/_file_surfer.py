...
class FileSurfer(BaseChatAgent):
    ...
    DEFAULT_DESCRIPTION = "An agent that can handle local files. Note that it is the web surfer that is responsible for downloading files. ONLY GIVE IT FILES THAT HAVE ALREADY BEEN DOWNLOADED WITH ABSOLUTE PATHS."
    ...
    DEFAULT_SYSTEM_MESSAGES = [
        ...
        Any downloaded files you are asked to open are stored in the browser data directory.
        """
        ),
    ]
    ...
    async def on_messages(self, messages: Sequence[ChatMessage], cancellation_token: CancellationToken) -> Response:
        for chat_message in messages:
            if isinstance(chat_message, TextMessage | MultiModalMessage):
                ...
                raise ValueError(
                    f"Unexpected message in FileSurfer: {chat_message}")
