...
class EvaluateRequestIsSatisfied(BaseModel):
    reason: str
    answer: bool
...
class EvaluateRequestIsInLoop(BaseModel):
    reason: str
    answer: bool
...
class EvaluateRequestProgress(BaseModel):
    reason: str
    answer: bool
...
class EvaluateRequestNextSpeaker(BaseModel):
    reason: str
    answer: str
...
class EvaluateRequestInstruction(BaseModel):
    reason: str
    answer: str
...
class EvaluateRequestParameters(BaseModel):
    is_request_satisfied: EvaluateRequestIsSatisfied
    is_in_loop: EvaluateRequestIsInLoop
    is_progress_being_made: EvaluateRequestProgress
    next_speaker: EvaluateRequestNextSpeaker
    instruction_or_question: EvaluateRequestInstruction
...
class ResponseFormat(BaseModel):
    parameters: EvaluateRequestParameters
                        ...
                        "properties": {
                            "is_request_satisfied": {
                                ...
                                "properties": {
                                    "reason": {
                                        "type": "string",
                                        "description": "Explanation for the satisfaction of the request"
                                    },
                                    "answer": {
                                        "type": "boolean",
                                        "description": "Indicates whether the request is satisfied"
                                    }
                                },
                            ...
                            "is_in_loop": {
                                ...
                                "properties": {
                                    "reason": {
                                        "type": "string",
                                        "description": "Explanation for whether the system is in a loop"
                                    },
                                    "answer": {
                                        "type": "boolean",
                                        "description": "Indicates whether the system is in a loop"
                                    }
                                },
                                "required": ["reason", "answer"],
                                "additionalProperties": False
                            ...
                            "is_progress_being_made": {
                                ...
                                "properties": {
                                    "reason": {
                                        "type": "string",
                                        "description": "Explanation for whether progress is being made"
                                    },
                                    "answer": {
                                        "type": "boolean",
                                        "description": "Indicates whether progress is being made"
                                    }
                                },
                                "required": ["reason", "answer"],
                                "additionalProperties": False
                            ...
                            "next_speaker": {
                                ...
                                "properties": {
                                    "reason": {
                                        "type": "string",
                                        "description": "Explanation for the choice of the next speaker"
                                    },
                                    "answer": {
                                        "type": "string",
                                        "description": "The name of the next speaker"
                                    }
                                },
                                "required": ["reason", "answer"],
                                "additionalProperties": False
                            ...
                            "instruction_or_question": {
                                ...
                                "properties": {
                                    "reason": {
                                        "type": "string",
                                        "description": "Explanation of the instruction or question"
                                    },
                                    "answer": {
                                        "type": "string",
                                        "description": "The instruction or question posed"
                                    }
                                },
                                "required": ["reason", "answer"],
                                "additionalProperties": False
...
class MagenticOneOrchestratorPlus(BaseGroupChatManager):
    ...
    def __init__(
        ...
        self._name = "MagenticOneOrchestratorPlus"
    ...
    async def handle_start(self, message: GroupChatStart, ctx: MessageContext) -> None:  # type: ignore
        ...
        self._task = " ".join([content_to_str(msg.content)
    ...
    async def _orchestrate_step(self, cancellation_token: CancellationToken) -> None:
        ...
        # Log model info
        if self._n_rounds == 0:  # Only log once at start
            await self._log_message(f"Starting orchestration with model: {self._model_client}")
        ...
        for _ in range(self._max_json_retries):
            response = await self._model_client.create(
                context,
                json_output=True,
                extra_create_args=self._merge_extra_create_args(
                    {"response_format": ResponseFormat})
            )
            ledger_str = response.content
            try:
                assert isinstance(ledger_str, str)
                parsed_response = json.loads(ledger_str)
                progress_ledger = parsed_response["parameters"]
                # Log the decision making
                await self._log_message(f"\nORCHESTRATOR DECISION:\n{json.dumps(progress_ledger, indent=2)}")
    ...
    async def _prepare_final_answer(self, reason: str, cancellation_token: CancellationToken) -> None:
        ...
        # Signal termination
        await self.publish_message(
            GroupChatTermination(message=StopMessage(
                content=reason, source=self._name)),
            topic_id=DefaultTopicId(type=self._output_topic_type),
        )
        if self._termination_condition is not None:
            await self._termination_condition.reset()
    ...
    def _merge_extra_create_args(self, hardcoded_args: Dict[str, Any]) -> Dict[str, Any]:
        """Merge client's extra_create_args with hardcoded args, prioritizing hardcoded ones.

        Args:
            hardcoded_args: Dictionary of hardcoded arguments that should override client args

        Returns:
            Dict containing merged arguments with hardcoded ones taking precedence
        """
        client_args = getattr(self._model_client, 'config',
                              {}).get('extra_create_args', {})
        return {
            **client_args,  # Client args come first
            **hardcoded_args  # Hardcoded args override
        }
